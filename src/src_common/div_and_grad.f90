...
contains
 ...
 !      do i=1,nfabs(phi(n))
  ...
  subroutine compute_grad_4o(mla,phi,gradp,dx, &
    ...
    call build(bpt,"compute_grad_4o")
 ...
 !      do i=1,nfabs(phi(n))
          ...
          do comp=start_incomp,start_incomp+num_comp-1
             ...
             select case (dm)
             case (2)
                call compute_grad_4o_2d(pp(:,:,1,comp), ng_p, &
                                     gpx(:,:,1,outcomp), gpy(:,:,1,outcomp), ng_g, &
                                     lo, hi, dx(n,:), &
                                     the_bc_level(n)%adv_bc_level_array(i,:,:,bccomp),xlo,xhi,ylo,yhi)
             case (3)
                gpz => dataptr(gradp(n,3), i)
                call compute_grad_4o_3d(pp(:,:,:,comp), ng_p, &
                                     gpx(:,:,:,outcomp), gpy(:,:,:,outcomp), gpz(:,:,:,outcomp), ng_g, &
                                     lo, hi, dx(n,:), &
                                     the_bc_level(n)%adv_bc_level_array(i,:,:,bccomp),xlo,xhi,ylo,yhi,zlo,zhi)
             end select
  ...
  contains
    ...
    subroutine compute_grad_4o_2d(phi,ng_p,gpx,gpy,ng_g,lo,hi,dx,bc,xlo,xhi,ylo,yhi)
      ...
      do j=xlo(2),xhi(2)
         do i=xlo(1),xhi(1)
            gpx(i,j) = ( -phi(i+1,j) + 15.d0*phi(i,j) - 15.d0*phi(i-1,j) + phi(i-2,j) ) * twelvedxinv
         end do
      end do
      ...
      do j=ylo(2),yhi(2)
         do i=ylo(1),yhi(1)
            gpy(i,j) = ( -phi(i,j+1) + 15.d0*phi(i,j) - 15.d0*phi(i,j-1) + phi(i,j-2) ) * twelvedxinv
         end do
      end do

    end subroutine compute_grad_4o_2d

    subroutine compute_grad_4o_3d(phi,ng_p,gpx,gpy,gpz,ng_g,lo,hi,dx,bc,xlo,xhi,ylo,yhi,zlo,zhi)
  ...
  end subroutine compute_grad_4o
