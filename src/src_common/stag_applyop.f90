module stag_applyop_module
  ...
  public :: stag_applyop, stag_applyop_level, stag_applyop_4o, stag_applyop_4o_level
...
!    do i=1,nfabs(phi_fc(1))
  ...
  ! compute Lphi
  subroutine stag_applyop_4o(mla,the_bc_tower,phi_fc,Lphi_fc,alpha_fc, &
                          beta_cc,beta_ed,gamma_cc,theta_alpha,dx)
    ...
    call build(bpt,"stag_applyop_4o")
    ...
    do n=1,nlevs
       call stag_applyop_4o_level(mla%la(n),the_bc_tower%bc_tower_array(n), &
                               phi_fc(n,:),Lphi_fc(n,:),alpha_fc_temp(n,:), &
                               beta_cc(n),beta_ed(n,:),gamma_cc(n),dx(n,:))
    end do
  ...
  end subroutine stag_applyop_4o

  ! compute Lphi
  subroutine stag_applyop_4o_level(la,the_bc_level,phi_fc,Lphi_fc,alpha_fc,beta_cc, &
                                beta_ed,gamma_cc,dx,color_in)
    ...
    call build(bpt,"stag_applyop_4o_level")

    if (dx(1) .ne. dx(2)) then
       call bl_error("stag_applyop_4o_2d requires the same dx in all directions")
    end if
...
!    do i=1,nfabs(phi_fc(1))
  ...
  subroutine stag_applyop_4o_2d(phix,phiy,ng_p,Lpx,Lpy,ng_l, &
    ...
    if (visc_type .eq. 1) then
       ...
       if (do_x) then
          ...
          do j=xlo(2),xhi(2)
             ...
             do i=xlo(1)+ioff,xhi(1),offset
                ...
                Lpx(i,j) = (alphax(i,j) + 60.d0*b*twelvedxsqinv)*phix(i,j) &
                     -(+ 16.d0*phix(i+1,j  ) &
                       + 16.d0*phix(i-1,j  ) &
                       -  1.d0*phix(i+2,j  ) &
                       -  1.d0*phix(i-2,j  ) &
                       ...
                       + 16.d0*phix(i  ,j-1) &
                       -  1.d0*phix(i  ,j+2) &
                       -  1.d0*phix(i  ,j-2) )*b*twelvedxsqinv
       ...
       if (do_y) then
          ...
          do j=ylo(2),yhi(2)
             ...
             do i=ylo(1)+ioff,yhi(1),offset
                ...
                Lpy(i,j) = (alphay(i,j) + 60.d0*b*twelvedxsqinv)*phiy(i,j) &
                     -(+ 16.d0*phiy(i+1,j  ) &
                       + 16.d0*phiy(i-1,j  ) &
                       -  1.d0*phiy(i+2,j  ) &
                       -  1.d0*phiy(i-2,j  ) &
                       ...
                       + 16.d0*phiy(i  ,j-1) &
                       -  1.d0*phiy(i  ,j+2) &
                       -  1.d0*phiy(i  ,j-2) )*b*twelvedxsqinv
       ...
       call bl_error("stag_applyop_4o_2d only supports visc_type=1")

    end if

  end subroutine stag_applyop_4o_2d

  subroutine stag_applyop_4o_3d(phix,phiy,phiz,ng_p,Lpx,Lpy,Lpz,ng_l, &
                             alphax,alphay,alphaz,ng_a,beta,ng_b, &
                             beta_xy,beta_xz,beta_yz,ng_e, &
                             gamma,ng_g,glo,ghi,dx,color,xlo,xhi,ylo,yhi,zlo,zhi)
  ...
  end subroutine stag_applyop_4o_3d
