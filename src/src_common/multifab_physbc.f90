module multifab_physbc_module
  ...
  public :: multifab_physbc, multifab_physbc_ho
...
contains
  ...
  ! multifab_physbc_ho:     Fills in two cell-averaged ghost cells by using quartic extrapolation using the
  !                         boundary condition and four interior points.  We are computing the values 
  !                         extrapolated to the ghost cell-averages  For FOEXTRAP/HOEXTRAP for now
  !                         we assume homogeneous Neumann.  Only works for pressure at walls currently.
  ...
  subroutine physbc_2d(s,lo,hi,ng,bc,bccomp,dx)
    ...
    real(kind=dp_t) :: x,y
    real(kind=dp_t) :: threeEighths,nineEighths,oneEighth
...
!!!!!!!!!!!!!!!!!!
  ...
  subroutine multifab_physbc_ho(s,start_scomp,start_bccomp,num_comp,the_bc_level, &
                                time_in,dx_in,prob_lo_in,prob_hi_in,increment_bccomp_in)
    ...
    call build(bpt,"multifab_physbc_ho")
    ...
    do i=1,nfabs(s)
       ...
       do scomp=start_scomp,start_scomp+num_comp-1
          ...
          select case (dm)
          case (2)
             call physbc_ho_2d(sp(:,:,1,scomp), lo, hi, ng, &
                            the_bc_level%adv_bc_level_array(i,:,:,bccomp),bccomp, dx)
          case (3)
             call physbc_ho_3d(sp(:,:,:,scomp), lo, hi, ng, &
                            the_bc_level%adv_bc_level_array(i,:,:,bccomp),bccomp, dx)
          end select
       end do
    end do
  ...
  end subroutine multifab_physbc_ho

  subroutine physbc_ho_2d(s,lo,hi,ng,bc,bccomp,dx)
    ...
    if (bc(1,1) .eq. FOEXTRAP) then
       ...
       do j=lo(2),hi(2)
          ...
          s(lo(1)-2,j) = ( &
                           ...
                           +  29.d0*s(lo(1)+1,j) &
                           -  15.d0*s(lo(1)+2,j) &
                           +   3.d0*s(lo(1)+3,j) ) / 2.d0
       end do
    else if (bc(1,1) .eq. INTERIOR) then
       ! either periodic or interior; do nothing
    else
       print *,'physbc_ho_2d: bc(1,1) =',bc(1,1),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
    ...
    if (bc(1,2) .eq. FOEXTRAP) then
       ...
       do j=lo(2),hi(2)
          ...
          s(hi(1)+2,j) = ( &
                           ...
                           +  29.d0*s(hi(1)-1,j) &
                           -  15.d0*s(hi(1)-2,j) &
                           +   3.d0*s(hi(1)-3,j) ) / 2.d0
       end do
    else if (bc(1,2) .eq. INTERIOR) then
       ! either periodic or interior; do nothing
    else
       print *,'physbc_ho_2d: bc(1,2) =',bc(1,2),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
    ...
    if (bc(2,1) .eq. FOEXTRAP) then
       ...
       do i=lo(1),hi(1)
          ...
          s(i,lo(2)-2) = ( &
                           ...
                           +  29.d0*s(i,lo(2)+1) &
                           -  15.d0*s(i,lo(2)+2) &
                           +   3.d0*s(i,lo(2)+3) ) / 2.d0
       end do
    else if (bc(2,1) .eq. INTERIOR) then
       ! either periodic or interior; do nothing
    else
       print *,'physbc_ho_2d: bc(2,1) =',bc(2,1),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
    ...
    if (bc(2,2) .eq. FOEXTRAP) then
       ...
       do i=lo(1),hi(1)
          ...
          s(i,hi(2)+2) = ( &
                           ...
                           +  29.d0*s(i,hi(2)-1) &
                           -  15.d0*s(i,hi(2)-2) &
                           +   3.d0*s(i,hi(2)-3) ) / 2.d0
       end do
    else if (bc(2,2) .eq. INTERIOR) then
       ! either periodic or interior; do nothing
    else
       print *,'physbc_ho_2d: bc(2,2) =',bc(2,2),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if

  end subroutine physbc_ho_2d

  subroutine physbc_ho_3d(s,lo,hi,ng,bc,bccomp,dx)
       ...
       do k=lo(3)-ng,hi(3)+ng
          ...
          do j=lo(2)-ng,hi(2)+ng
             ...
             s(lo(1)-2,j,k) = ( &
                                ...
                                +  29.d0*s(lo(1)+1,j,k) &
                                -  15.d0*s(lo(1)+2,j,k) &
                                +   3.d0*s(lo(1)+3,j,k) ) / 2.d0
          end do
       end do
    else if (bc(1,1) .eq. INTERIOR) then
       ! either periodic or interior; do nothing
    else
       print *,'physbc_ho_3d: bc(1,1) =',bc(1,1),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
       ...
       do k=lo(3)-ng,hi(3)+ng
          ...
          do j=lo(2)-ng,hi(2)+ng
             ...
             s(hi(1)+2,j,k) = ( &
                                ...
                                +  29.d0*s(hi(1)-1,j,k) &
                                -  15.d0*s(hi(1)-2,j,k) &
                                +   3.d0*s(hi(1)-3,j,k) ) / 2.d0
          end do
       end do
    else if (bc(1,2) .eq. INTERIOR) then
       ! either periodic or interior; do nothing
    else
       print *,'physbc_ho_3d: bc(1,2) =',bc(1,2),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
...
!!!!!!!!!!!!!!!!!!
    ...
    if (bc(2,1) .eq. FOEXTRAP) then
       ...
       print *,'physbc_ho_3d: bc(2,1) =',bc(2,1),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
...
!!!!!!!!!!!!!!!!!!
    ...
    if (bc(2,2) .eq. FOEXTRAP) then
       ...
       print *,'physbc_ho_3d: bc(2,2) =',bc(2,2),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
...
!!!!!!!!!!!!!!!!!!
    ...
    if (bc(3,1) .eq. FOEXTRAP) then
       ...
       print *,'physbc_ho_3d: bc(3,1) =',bc(3,1),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if
...
!!!!!!!!!!!!!!!!!!
    ...
    if (bc(3,2) .eq. FOEXTRAP) then
       ...
       print *,'physbc_ho_3d: bc(3,2) =',bc(3,2),' for bccomp =',bccomp
       call bl_error('NOT SUPPORTED')
    end if

  end subroutine physbc_ho_3d
